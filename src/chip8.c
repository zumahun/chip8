...
static void chip8_exec_extended_eight(struct chip8* chip8, unsigned short opcode)
{
	...
	switch(final_fourbits)
	{
		...
		// 8xy4 - ADD Vx, Vy, set Vx = Vx + Vy, st VF = carry;
		case 0x04:
...
static void chip8_exec_extended_F(struct chip8* chip8, unsigned short opcode)
{
	...
	switch (opcode & 0x00ff)
	{
		...
		case 0x55:
		{
			for (int i = 0; i <= x; i++)
			{
				chip8_memory_set(&chip8->memory, chip8->registers.I+i, chip8->registers.V[x]);
			}
		}
		break;
...
static void chip8_exec_extended(struct chip8* chip8, unsigned short opcode)
{
	...
	switch(opcode & 0xf000)
	{
		...
		// CALL addr,2nnn Call subroutine at locaton nnn
		case 0x2000:
